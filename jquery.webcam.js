(function(){
		...
		flash_notify: function(type, msg) {
			switch (type) {
				case 'flashLoadComplete':
					...
					cfg.loaded = true;
					cfg.fire_hook('onLoad');
					break;

				case 'error':
					// HTTP POST error most likely
					if (!cfg.fire_hook('onError', msg)) {
				...
				case 'success':
					...
					cfg.fire_hook('onComplete', msg.toString());
					break;
	...
	$.fn.photo = function(opts){
		...
		return this.each(function(){
			cfg.set_api_url(options.api_url);
			cfg.set_quality(options.quality); // JPEG quality (1 - 100)
			cfg.set_shutter_sound(true); // play shutter click sound
